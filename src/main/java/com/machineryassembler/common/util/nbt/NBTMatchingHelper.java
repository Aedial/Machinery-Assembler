...
// Based on hellfirepvp/modularmachinery/common/util/nbt/NBTMatchingHelper.java from MMCE
...
/**
 * Helper for matching NBT compounds with pattern-based matching support.
 */
public class NBTMatchingHelper {
    ...
    public static boolean matchNBTCompound(@Nullable NBTTagCompound matchNBT, @Nullable NBTTagCompound itemStackNBTToCheck) {
        if (matchNBT == null) return true;

        if (itemStackNBTToCheck == null) return matchNBT.getKeySet().isEmpty();
    ...
    private static boolean matchCompound(NBTTagCompound matchNBT, NBTTagCompound itemStackNBTToCheck) {
        for (String keyMatch : matchNBT.getKeySet()) {
            if (!itemStackNBTToCheck.hasKey(keyMatch)) return false;

            NBTBase baseOriginal = matchNBT.getTag(keyMatch);
            NBTBase baseStack = itemStackNBTToCheck.getTag(keyMatch);

            if (!matchBase(baseOriginal, baseStack)) return false;
        }
    ...
    private static boolean matchList(NBTTagList baseOriginal, NBTTagList baseStack) {
        if (baseOriginal.tagCount() == 0) return true;

        if (baseStack.tagCount() == 0 || baseOriginal.getTagType() != baseStack.getTagType()) return false;
